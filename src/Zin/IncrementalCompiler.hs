...
import Zin.AST
import Zin.Cache (CacheKey (..), CacheManager (..), CachePolicy (..), createCacheManager, defaultCachePolicy, getCacheSize, getCacheStats, insertCache, lookupCache, optimizeCache)
import Zin.Chunking
import Zin.Diff
import Zin.Error
...
performIncrementalCompilation oldSource newSource compiler startTime = do
  ...
  (compiledNew, reusedExisting, newState, newCache, errors) <-
    recompileChangedChunks
      changedChunks
      (lastChunks compiler)
      newChunks
      (compilerState compiler)
      (cacheManager compiler)
...
identifyChangedChunks oldChunks newChunks changes =
  let changedIds = extractChangedIds changes
      ...
      changedInNew =
        filter
          ( \c ->
              chunkId c `elem` changedIds
                || chunkId c `Map.notMember` oldMap
          )
          newChunks
   in changedInNew
      ...
      return $
        T.concat
          [ "Performance Analysis:\n",
            ...
            T.pack $ show (avgHitRate * 100),
            "%\n",
            "Total chunks processed: ",
            T.pack $ show totalProcessed,
            "\n",
            "Total chunks reused: ",
            T.pack $ show totalReused,
            "\n",
