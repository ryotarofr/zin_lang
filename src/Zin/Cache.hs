    ...
    Just entry -> do
      ...
      case entryTTLExpiry metadata of
        Just expiry | now > expiry -> do
          ...
          let newStats = (cacheStats manager) 
...
insertCache key chunk manager = liftIO $ do
  ...
  let ttlExpiry = if entryTTL policy > 0
...
enforcePolicy manager = do
  ...
  -- エントリ数制限のチェック
  manager1 <- if currentEntries > maxCacheEntries policy
              then evictEntries (currentEntries - maxCacheEntries policy) manager
              else return manager
  
  -- メモリ制限のチェック
  manager2 <- if currentMemory > maxCacheMemory policy
