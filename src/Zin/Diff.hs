...
calculateDP oldVec newVec i j
  ...
  | otherwise = 
      1 + minimum [ calculateDP oldVec newVec (i-1) j     -- deletion
...
calculateDiffStatistics lineChanges chunkChanges = 
  ...
  in DiffStatistics addedLines deletedLines modifiedLines 
                    addedChunks deletedChunks modifiedChunks unchangedChunks
...
levenshteinHelper (c1:s1) (c2:s2) = 
  ...
  else 1 + minimum [ levenshteinHelper s1 (c2:s2)    -- deletion
